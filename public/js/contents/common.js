...
app.common = function() {
    ...
    function getGroupId() {
        var groupId = $(".js-contents").attr("data-group-id");
        if ( groupId ) {
            return groupId;
        }
        throw callback + " can not get the group_id";
    }
    that.getGroupId = getGroupId;

    function getAppId() {
        var appId = $(".js-contents").attr("data-app-id");
        if ( appId ) {
            return appId;
        }
        return false;
    ...
    function showMessage(classType, text) {
        var $div = $("<div>").addClass("js-system-message system-message style-display-none "+ classType).html(text);
        $(".js-system-message").remove();
        $("body").append($div);
        $div.fadeIn();
